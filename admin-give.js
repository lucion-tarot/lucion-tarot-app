...
module.exports=async (req,res)=>{
  try{
    ...
    const u=await dbGet('/users/'+p)||{phone:p,credits:0,issued:0,freeUsed:false};
    const newCredits=Number(u.credits||0)+add, issued=Number(u.issued||0)+add;
    await dbPatch('/users/'+p,{phone:p,maskedPhone:maskPhone(p),maskedName:u.maskedName||maskName(u.name||''),credits:newCredits,issued,updatedAt:Date.now()});
